    ...
    */
    ...
    public function ekle(){
        ...
        if ($product_name != null){
            $deger = $this->model_products->create($data);
            echo $deger;
            $this->session->set_flashdata('message', 'success:' . $this->lang->line('application_stock_addproduct_saved'));
    ...
    */
    public function fetchProductData()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $availability = ($value['availability'] == 1) ? '<span class="label label-success">Aktif</span>' : '<span class="label label-warning">Inactive</span>';

            $qty_status = '';
            if($value['qty'] <= 10) {
                $qty_status = '<span class="label label-warning">Düşük Stok !</span>';
            } else if($value['qty'] <= 0) {
                $qty_status = '<span class="label label-danger">Stokta Yok !</span>';
            }
    ...
    public function update($product_id)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if($update == true) {
                ...
                $this->session->set_flashdata('errors', 'Database Hata!!');
           ...
           // $this->content_view = '/products/edit' . $this->data ;
            $this->render_template('blueline/products/edit', $this->data);
            //print_r($this->data);
        }
    }
